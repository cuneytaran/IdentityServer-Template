...
namespace UdemyIdentityServer.Client2.Services
{
    public class ApiResourceHttpClient : IApiResourceHttpClient
    {
        ...
        public async Task<List<string>> SaveUserViewModel(UserSaveViewModel userSaveViewModel)
        {
            ...
            clientCredentialsTokenRequest.ClientId = _configuration["Client2Mvc:ClientId"];//appsettingsden ClientId alıyoruz configurationdan
            clientCredentialsTokenRequest.ClientSecret = _configuration["Client2Mvc:ClientSecret"];//appsettingsden ClientSecret alıyoruyoruz
